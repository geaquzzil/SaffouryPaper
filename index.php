...
//	echo "AIT";
    if(!authfunction(getAuthHeader()))returnAuthErrorMessage("Authorization error");
    //	echo "END AIT";
    setUser();
	//Checking request
	checkRequest();
...
function checkAction(){
	...
	switch (getRequestValue('action')) {
		...
		break;
		case 'delete':
		returnResponse( deleteObject(checkRequestValue('iD')?getRequestValue('iD'):getRequestValue('data'),
		getRequestValue('table'),checkRequestValue('notify')?getRequestValue('notify'):true));
				//delete
			//checkPermission
		break;
   ...
   // print_r($tableColumns);
	...
	foreach(array_keys($requestAttributes) as $ra){
		if(isTableColumn($ra)){
			...
			($tableColumns)) ){returnBadRequest("$ra is not founded in table $tableName ");}
			else{
				if(empty(getRequestValue($ra))){
					returnBadRequest("$ra is empty ");
				}
