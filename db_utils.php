...
function getAllTablesWithoutViewString(){
	return getStrings("SELECT table_name FROM information_schema.tables WHERE table_schema ='".DATABASE_NAME."' AND TABLE_TYPE <> 'VIEW' "
	,"TABLE_NAME");
...
function getStrings($query,$value)	{
	...
	if(!is_null($result)){if(is_array($result)){foreach($result as $r){array_push($response,$r[$value]);}}}
	return $response;
}
...
function getSearchObjectStringValue($object,$tableName){
	...
	$forginsKey = array_map(function($tmp) { return  $tmp["COLUMN_NAME"]; }, $forgins);
