...
class webSocketServer
{
    ...
    function __construct($Address, $certFile = '', $pkFile = '')
    {
        ...
        while ($a) {
            ...
            foreach ($socketArrayRead as $Socket) {
                ...
                if ($Socket === $this->socketMaster) {
                    ...
                    $this->Log("Connecting from IP: $ip->ip  $ipport");
                ...
                if ($Client->Handshake) {
                    ...
                    $this->log("is Handshake message = $message ");
                    if ($message != '') {
                        /*
                         * ***********************************************
                ...
                if (
                    $dataBuffer === false ||
                    strlen($dataBuffer) == 0 ||
                    strlen($dataBuffer) >= $this->bufferChunk
                ) {  // to avoid malicious overload 
    ...
    {
        $uns = substr($uuHash, 0, 36);
        $hash = substr($uuHash, 36);
        $token = ''; // generate whatever yo want 
    ...
    { // ...socket has been closed AND deleted
