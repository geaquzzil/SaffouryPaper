...
function convertToObject(&$object){
    if(is_array($object)){
      //  echo "\n object is array convert to object \n";
        $soso = new stdClass();
        foreach ($object as $key => $value){ $soso->$key = $value; }
        $object=$soso;
    }
}
//end utils

function curdate() {return date('Y-m-d');}
...
function jsonDecode($jsonPost){return json_decode($jsonPost,true);}
...
function getRequestValue($key){
	$method = getReqestMethod();
		switch ($method) {
			case 'GET':	return $_GET[$key];		
			case 'POST':return $_POST[$key];	
			case 'DELETE':break;
			case 'PUT':	break;
		}
}
...
function isActionTableIsP($tableName,$key){return $key===PARENTID ? false :isActionTableIs($tableName);  }
function isActionTableIs($tableName){ return getRequestValue('table')===$tableName;}
function isActionIsView(){return getRequestValue('action')==='view';}
function isActionIsList(){return getRequestValue('action')==='list';}
function isActionIsDelete(){return getRequestValue('action')==='delete';}
function isActionIs($actionName){return getRequestValue('action')===$actionName;}
...
function checkRequestValue($key){
	$method = getReqestMethod();
	switch ($method) {
			case 'GET':	return isset($_GET[$key]) && !IsNullOrEmptyString($_GET[$key]);		
			case 'POST':return isset($_POST[$key])&& !IsNullOrEmptyString($_POST[$key]);	
			case 'DELETE':break;
			case 'PUT':	break;
		}
}
...
//	print_r($myServerArgs);
	...
	$method = getReqestMethod();
	switch($method){
		case 'GET':	return $myGetArgs;		
		case 'POST':return $myPostArgs;	
		case 'DELETE':break;
		case 'PUT':	break;
	}
}
