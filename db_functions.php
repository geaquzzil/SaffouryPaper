...
function getFetshTableWithQueryOptions($iD, $tableName, $option)
{
	...
	if (isset($option["WHERE_EXTENSION"])) {
		$newQuery = has_word($newQuery, "WHERE") ?
			($newQuery . " AND " . $option["WHERE_EXTENSION"]) : ($newQuery . " WHERE " . $option["WHERE_EXTENSION"]);

		//echo$newQuery;
	}
	if (isset($option["SEARCH_QUERY"])) {
		$newQuery = has_word($newQuery, "WHERE") ?
			($newQuery . " AND " . $option["SEARCH_QUERY"]) : ($newQuery . " WHERE " . $option["SEARCH_QUERY"]);
...
function getQuery($tableName, $ID)
{
	switch ($tableName) {
		case CUST:
			return "SELECT name,iD,phone FROM  `" . addslashes($tableName) . "` WHERE iD=$ID";
		case USR_L:
			return "SELECT * FROM `" . addslashes($tableName) . "` WHERE iD=$ID";
		case DEAL:
			return "SELECT name,iD,phone FROM `" . addslashes($tableName) . "` WHERE iD=$ID";
		case EMP:
			return "SELECT name,iD,userlevelid FROM  `" . addslashes($tableName) . "` WHERE iD=$ID";
		default:
			return "SELECT * FROM  `" . addslashes($tableName) . "` WHERE iD=$ID";
	}
}
...
{
	if (ctype_digit($valueToSearch)) {
		return "SELECT * FROM  " . addslashes($tableName) . " WHERE $columnName=$valueToSearch";
	} else {
		return "SELECT * FROM  " . addslashes($tableName) . " WHERE $columnName = '$valueToSearch'";
	}
}
function getQueryName($tableName)
{
	switch ($tableName) {
		case EMP:
			return "SELECT iD,name,userlevelid,CONVERT(phone,char) AS phone FROM  " . addslashes($tableName) . " ORDER BY name ASC";
		case CUST:
			return "SELECT name,iD,phone,token FROM  " . addslashes($tableName) . " ORDER BY name ASC";
		case COUNTRY:
			return "SELECT * FROM  " . addslashes($tableName) . " ORDER BY name ASC";
		case AC_NAME:
			return "SELECT * FROM  " . addslashes($tableName) . " ORDER BY name ASC";
		case TYPE:
			return "SELECT * FROM  " . addslashes($tableName) . " ORDER BY name ASC";
		case GSM:
			return "SELECT * FROM  " . addslashes($tableName) . " ORDER BY gsm ASC";
		default:
			return "SELECT * FROM  " . addslashes($tableName) . " ";
	}
}
