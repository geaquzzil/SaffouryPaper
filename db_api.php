...
function getNotUsedRecords($tableName){
	...
	$results=array();
	if(is_null($forginsDetails)||empty($forginsDetails)){return array();}
	$i=0;
...
function recursiveSerachObject($recursiveObj,$parentKey,$object,$tableName,$recursiveLevel){
    $recursiveObj=$object;
	if($recursiveLevel===0){return $recursiveObj;}
...
//	echo "done search \n";
	if(isset($arr['iD'])){$iD=$arr['iD'];unset($arr['iD']);} 
	if(is_null($iD)|| $iD==-1){
	//	echo "insert \n";
		//SET FB EDIT
		// insert
...
function validateJsonColsAndAdd($Parent,$isParent,&$object,$tableName,$option){
 ...
 // echo "\n adding \n";
	...
	if(empty($object)|| is_null($object)){return null;}
	$forginsDetails= getArrayForginKeys($tableName);
	$forginsDetails = array_map(function($tmp) { return getJsonKeyFromForginArray($tmp); }, $forginsDetails);
//	print_r($object);
//	print_r($forginsDetails);
	$forgins=getObjectForginKeys($tableName);
	$forgins = array_map(function($tmp) { return  getJsonKeyFromForginObject($tmp); }, $forgins);
	//	print_r($forgins);
	...
	if(!empty($DetailsToAdd)){
		foreach($DetailsToAdd as $detailsArray){
		    //echo "\n Detail to add \n"; 
			validateJsonArrayAndAdd($object,$tableName,
			($detailsArray),$detailsArray['currentTableName'],$option);
		}
	}
...
function searchByID($ParentTableName,$forginsDetails,$forgins,$obj,$tableName,$recursiveLevel,$detailArrayTable,$detailObjectTable,$options){
	...
	if(!empty($forgins)){	
		...
		foreach($forgins as $forgin){
		    ...
		    if(!is_null($skipedObject)){ if($pp==$skipedObject){continue; }}
		    if(isParent($forgin)){
		        if($RequireParent){
		             $Where=PARENTID;
		    ...
		    }else if(!isCurrentObjectIDEmpty($result,$forgin) && isDetailObjectRequire($ParentTableName,$forgin,$detailObjectTable)
			&& !isActionTableIs($pp) )
			{
...
function depthSearchByDetailTable($masterTableName,$detailTableName,$Column,$ColumnValue,$returnAllDetails){
  ...
  //  print_r($forginsDetails);
    ...
    $results = array_map(function($tmp) { return $tmp['TABLE_NAME']; }, $forginsDetails);
	...
	$results = array_map(function($tmp) { return $tmp['COLUMN_NAME']; }, $forgins);
...
function depthSearch($iD,$tableName,$recursiveLevel,$detailArrayTable,$detailObjectTable,$options){
	if(is_null($iD)){
		...
		if(is_null($results)||empty($results)){return array();}
...
function deleteObject($object,$tableName,$sendNoti){
	...
	try {
		...
		if(empty($toDeleteObjects)){return null;}
		$responseArray=array();
...
function doDelete($object,$tableName,$sendNoti){
	try {
		...
		if($result){
		    ...
		    if($sendNoti){doNotification($object,$tableName,"delete");}
		}
		return $result;
