  ...
  //   echo "\n checkToAddRemoveJournal $tableName   ";
	...
	if(!isNewRecord($object)){	
	   // 	    echo "  OBJECT IS JOURNAL isNewRecord $tableName  ";
		...
		getKeyValueFromObj($object,'iD'),$tableName,0,null,null,null);	
...
$FIX_ADD_OBJECT[CRED]=function(&$object){checkToAddRemoveJournal($object,CRED);};
$FIX_ADD_OBJECT[DEBT]=function(&$object){checkToAddRemoveJournal($object,DEBT);};
	...
	if(property_exists($object,'status') && isset($object->status)){
		...
		if(is_numeric($status)){
	        switch($status){
	            ...
	            case 1:$unit='PENDING';break;
	            case 2:$unit='PROCESSING';break;
	            case 3:$unit='CANCELED';break;
	            case 4:$unit='APPROVED';break;
	        }
		$object->status=$status;
	    }
	}else{
