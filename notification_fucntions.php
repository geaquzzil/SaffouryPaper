...
define('FB_URL','https://fcm.googleapis.com/fcm/send');
...
function doNotification($object,$tableName,$action){
  //   echo "  sad ";
	...
	switch($tableName){
	    case FB_SETTING_CHANGED:send_notification_all($object,FB_SETTING_CHANGED);return; 
	}
    switch($action){
        case FB_GENERAL_NOTIFCATION:send_notification_all($object,$action);return;
		case FB_NEW_PRODUCT:send_notification_new($object,$action);return;
			// only enables on customers
		case FB_LOG_OUT:sendCustomer(null,$object,$action);return;
	...
	if (($i = array_search($tableName,$acceptedNotifications)) !== FALSE){
	 //   echo "  sad ";
		switch($action){
			case "delete":sendDelete($object,$tableName);break;
			//new -  edit
			default:sendNotificationProcess($object,$tableName);break;
		}
	}
}
function sendNotificationProcess($object,$tableName){
	...
	switch($tableName){
		...
		case ORDR:
			...
			break;	
	    case INC: case DEBT:   case CRED:	case SP:	
...
function sendDelete($object,$tableName){
	...
	switch($tableName){		
		...
		case ORDR_R:
		case ORDR:		
			sendAll(getFirebaseDetailsObject($object,$tableName),$tableName);break;	
		break;
		case SP:
		case INC:
		case DEBT:
