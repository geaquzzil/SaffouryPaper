...
function getSearchDataFromProductsOrCustomers($isProduct){
	...
	if(!checkRequestValue('searchQuery') ||
...
function getQueryFromAdvancedSearch($isSimilar){
	...
	if($Type=="All" OR $Type=="الكل"){$Type="";}else{$Type="AND `Type` = '$Type' ";}
	if($GSM=="All"){ $GSM="";}else{$GSM="AND `GSM` = '$GSM' ";}
	if($Unit=="Roll" OR $Unit=="رول" OR $Unit=="Reel"){$Unit="AND (`Length` ='0' OR `Length` IS NULL) ";}else if($Unit=="Pallet" OR $Unit=="بالة"){$Unit="AND `Length` <>'0' ";}else{ $Unit="";}
	//Date statement
	if($Date=="All" OR $Date=="الكل"){$Date="";}
	...
	if($Country=="All" OR $Country=="الكل"){$Country="";}else{$Country="AND `Country` = '$Country' ";}
	if($Quality=="All" OR $Quality=="الكل"){$Quality="";}else{$Quality="AND `Quality` = '$Quality' ";}
	if($Grade=="All" OR $Grade=="الكل"){$Grade="";}else{$Grade="AND `Grade` = '$Grade' ";}
	if($Warehouse=="All" OR $Warehouse=="الكل"){$Warehouse="";}else{$Warehouse="AND `Warehouse` = '$Warehouse' ";}
...
$API_ACTIONS["list_advanced_search"]=function(){
    if(!checkRequestValue('data')){returnBadRequest("Bad request");}
	if(!isJson(getRequestValue('data'))){returnBadRequest("Bad request");}
...
$API_ACTIONS["action_notification"]=function(){
	...
	if(is_null($iD)){
		doNotification($Message,null,FB_GENERAL_NOTIFCATION);
			...
			send_notification(getRegestrationsIDTable($customer,CUST),
...
//	if(checkDat)
	...
	if(is_null($customer) || empty($customer)){ return null;}// no content}
	$FROM = date("Y-m-d",strtotime($DateOrMonth['from']));
	$TO = date("Y-m-d",strtotime($DateOrMonth['to']));
	$E_FROM= date('Y-m-d',(strtotime ( '-1 day' , strtotime ( $FROM) ) ));
	...
	if((!empty($customer[ORDR]) || !is_null($customer[ORDR]))){	
		$results = array_map(function($tmp) { return $tmp['iD']; }, $customer[ORDR]);
		$op["KEY"]="OrderID";
	...
	if((!empty($customer[PURCH]) || !is_null($customer[PURCH]))){	
		$results = array_map(function($tmp) { return $tmp['iD']; }, $customer[PURCH]);
		$op["KEY"]="PurchaseID";
...
$API_ACTIONS["list_not_used_records"]=function(){
    ...
    if(!isAdmin()){  returnPermissionResponse($table,0);}
	...
	switch($tableName){
	   ...
	   case ORDR:
		...
		case PURCH:
		  ...
		  return "quantity";
		case INC:case SP:case DEBT:case CRED:
		    //fromBox isDirect
		  return "value";
		default:
		  return null;
...
//	print_r($permission);
	if(isEmployee() || isCustomer()){
		...
		$option["WHERE_EXTENSION"]= $IsDate ? 
			("Date(`date`) = '$TO' ORDER BY `date` DESC ") :
			("Month(`date`) = '$TO' ORDER BY `date` DESC ");
		...
		if( (!empty($response[ORDR]) || !is_null($response[ORDR])) && isCustomer() ){	
			$results = array_map(function($tmp) { return $tmp['iD']; }, $response[ORDR]);
			$op["KEY"]="OrderID";
		...
		if( (!empty($response[PURCH]) || !is_null($response[PURCH])) && isCustomer() ){	
			$results = array_map(function($tmp) { return $tmp['iD']; }, $response[PURCH]);
			$op["KEY"]="PurchaseID";
